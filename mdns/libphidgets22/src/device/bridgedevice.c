...
PhidgetBridgeDevice_dataInput(PhidgetDeviceHandle device, uint8_t *buffer, size_t length) {
	...
	case PHIDUID_DAQ0501:
		...
		case VINT_PACKET_TYPE_VOLTAGERATIOINPUT_SATURATION:
			if ((channel = getChannel(phid, chIndex)) != NULL) {
				//Send out events
				res = bridgeSendToChannel(channel, BP_ERROREVENT, "%d%s", EEPHIDGET_SATURATION, "Saturation Detected.");
				PhidgetRelease(&channel);
				return res;
			}
			return EPHIDGET_OK;
...
static void PhidgetBridgeDevice_updateBridgeIntervals(PhidgetChannelHandle ch, PhidgetBridgeDeviceHandle phid, BridgePacket *bp)
{
	...
	//Need to update the other channels to reflect minimum interval with the new number of enabled channels
	enabledCount = 0;
	for (i = 0; i < phid->devChannelCnts.numBridgeInputs; i++) {
		if ((phid->enabled[i] != 0) && (phid->enabled[i] != PUNK_BOOL))
			enabledCount++;
	}
	if (enabledCount > 1)
		minInterval = 4 * enabledCount;
	else
		minInterval = 0;

	for (i = 0; i < phid->devChannelCnts.numBridgeInputs; i++) {
		if (phid->dataInterval[i] < minInterval)
			interval = minInterval;
		else if (phid->dataInterval[i] != PUNK_UINT32)
